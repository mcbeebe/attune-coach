...
- **3.7 Session library seeded from established Hyrox training practice.** Two practitioner guides supplied by the product owner corroborate the P3 design and contribute concrete session shapes (summaries via search index; full pages are egress-blocked from the dev environment — pull them directly when building):
  - [STRIDE Fitness — HYROX Training Guide (12-week plan + official race format)](https://www.stridefitness.com/community/hyrox-training-guide): strength 2–3×/wk built on compound lifts (squats, deadlifts, presses) plus Hyrox-specific work (sled push, walking lunges, farmer's carries); running 3×/wk including 4–6 × 800 m at race pace with 90 s rest; **combo work 1×/wk — run 1 km then complete a station immediately** "to train the leg-burn-into-strength transition"; class work concentrated in Z3–4.
  - [PureGym — Free Hyrox Training Plan (8- and 12-week)](https://www.puregym.com/blog/hyrox-training-plan/): weekly template of **Intervals · Strength &amp; skills station circuit · Compromised run · Easy run + mobility**; ~50% of race time is running, so endurance work gets parity with stations; race simulations and recovery built into the plan; per-station technique guides ([station tips](https://www.puregym.com/blog/hyrox-exercises-tips-and-tricks-for-each-station/), [running](https://www.puregym.com/blog/hyrox-running/)) usable as coaching copy per station.
  - What this changes in P3: the weekly role vocabulary should match the practitioner template above (interval run / station circuit / **compromised run** / easy run — confirming 3.4 as a first-class weekly session, not a peak-only event); 800 m race-pace repeats join 1 km repeats in the interval pool; the 12-week default runway (8-week compressed variant for experienced athletes) becomes the progression model's reference length; and equipment-substitution guidance (no sled/erg available) should ship with the station library so home-gym athletes get a valid plan.
